...
def Initialize(pMatrixUI, pClipLauncher):
    ...
    inputManager = InputManager.InputManager(None, None)
    # Construct Groove Matrix
    global g_GrooveMatrix
    g_GrooveMatrix = GrooveMatrix(pMatrixUI, pClipLauncher, InputManager)
...
def HandleEvent(pSdlEvent):
    ...
    if sdlEvent.type == sdl2.events.SDL_QUIT:
        g_GrooveMatrix.GetMatrixUI().SetQuitFlag(True)

import time
def Update():
    global g_GrooveMatrix
    g_GrooveMatrix.GetMatrixUI().Update()
    g_GrooveMatrix.GetMatrixUI().Draw()
