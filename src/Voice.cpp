...
Voice::Voice( const_ptr<Clip> pClip, int ID, size_t uTriggerRes, float fVolume, bool bLoop /*= false*/ ) :
	Voice()
{
	...
	if ( pClip && ID >= 0 )
	{
		...
		throw std::runtime_error( "Error: Invalid clip supplied to Voice constructor!" );
}

// Construct with a ClipLauncher::Command
Voice::Voice ( const ClipLauncher::Command cmd ) :
	Voice()
{
	// Save some words
	using ECommandID = ClipLauncher::ECommandID;

	// Check the validity of the command
	if ( cmd.eID == ECommandID::StartVoice || cmd.eID == ECommandID::OneShot )
	{
		if ( cmd.pClip && cmd.eID != ECommandID::None )
		{
			...
			m_fVolume = cmd.fData;
			m_eState = cmd.eID == ECommandID::StartVoice ? EState::Pending : EState::OneShot;
		}
	}
}
...
void Voice::RenderData( float * const pMixBuffer, const size_t uSamplesDesired, const size_t uSamplePos )
{
	...
	// worst case it goes from pending (1) => starting(the # of fade-ins + 1) => looping (# of filled buffers + 1)
	size_t uLoopIterations( 0 );
	const size_t uMaxLoopIterations = 3 + uFadeSamples / uSamplesDesired + uSamplesDesired / uSamplesInHead;

	// We loop here until the # of samples added matches the # desired
	// uSamplesAdded is incremented when samples at the end of each iteration
	for ( size_t uSamplesAdded = 0; uSamplesAdded < uSamplesDesired; uLoopIterations++ )
	{
		// I'd rather raise an exception than loop forever...
		if ( uLoopIterations >= uMaxLoopIterations )
		...
		// Because a voice can start playing at any position within the overall mix buffer,
		// transform the current sample pos into the sample pos relative to when we started
		size_t uMyCurrentSamplePos( 0 );
		if ( uCurrentSamplePos < m_uStartingPos )
			uMyCurrentSamplePos = uCurrentSamplePos + (uSamplesInHead - m_uStartingPos);
		else
			uMyCurrentSamplePos = uCurrentSamplePos - m_uStartingPos;

		// Now that we have the sample idx from our perspective, find the buffer idx
		size_t uFirstHeadSample = uMyCurrentSamplePos % uSamplesInHead;
		...
		if ( m_uLastTailSampleAdded < uTotalSampleCount )
		{
			...
			// Update the last tail sample added here
			...
			case EState::Starting:
				...
				if ( uFirstHeadSample < uFadeSamples )
				{
					...
					for ( ; uFirstHeadSample < uLastFadeFromZero; uFirstHeadSample++ )
					{
						...
						pMixBuffer[uSamplesAdded++] += remap<float>( (float) uFirstHeadSample, 0.f, (float) uFadeSamples, 0.f, fSampleVal );
					}
		...
		{
			// Fade the current sample to the target val, mix it in
			float fSampleVal = m_fVolume * pAudioData[uFadeIdx];
			float fFadedVal = remap( (float) uFadeIdx, (float) uFadeBegin, (float) uSamplesInHead, fSampleVal, fTargetVal );;
			pMixBuffer[uSamplesAdded++] += fFadedVal;
		}
